  ...
  while(go){
    if(itemCnt < 3){
      ...
      if (!map_array[x][y]){
        ...
        attron(COLOR_PAIR(9));
        mvprintw(20 + i, 80, "Gitem:(%d, %d)", x, y);
  ...
  while(go){
    if(itemCnt < 3){
      ...
      if (!map_array[x][y]){
        ...
        uniform_int_distribution<int> p(11, 15); // 아이템 출현 유지 기간 (11초~15초) 랜덤
        attron(COLOR_PAIR(9));
        mvprintw(30 + i, 80, "Pitem:(%d, %d)", x, y);
        attroff(COLOR_PAIR(9));
        sleep(p(gen));
        map_array[x][y] = 0;
        itemCnt--;
...
void makeGate(){
  ...
  while(go){
    ...
    if ((map_array[x1][y1] == 0 || map_array[x1][y1] == 1) && (map_array[x2][y2] == 0 || map_array[x2][y2] == 1)){
      ...
      g2.r = x2; g2.c = y2;
      attron(COLOR_PAIR(9));
      mvprintw(60, 80, "g1:(%d, %d), g2:(%d, %d)", g1.r, g1.c, g2.r, g2.c);
      attroff(COLOR_PAIR(9));
      sleep(7);
    ...
    //attron(COLOR_PAIR(9));
    // mvprintw(150, 80, "g1:(%d, %d), g2:(%d, %d)", g1.r, g1.c, g2.r, g2.c);
    // attroff(COLOR_PAIR(9));
